...
function tarefas() {
    ...
    async function consultarTarefas() {
        setTarefas(await login_backend.getTarefas());
    }

    async function consultarTotais() {
        const totalAndamento = parseInt(await login_backend.totalTarefasEmAndamento());
        const totalConcluidas = parseInt(await login_backend.totalTarefasConcluidas());
    ...
    async function handleSubmit(event) {
        ...
        if (idTarefa === null || idTarefa === "") {
            await login_backend.addTarefa(descricao, categoria, false, false);
        } else {
            await login_backend.alterarTarefa(parseInt(idTarefa), categoria, descricao, urgente, false);
        }
    ...
    async function excluir(id) {
        await login_backend.excluirTarefa(parseInt(id));
        consultarTarefas();
        consultarTotais();
    }

    async function alterar(id, categoria, descricao, urgente, concluida) {
        await login_backend.alterarTarefa(parseInt(id), categoria, descricao, urgente, concluida);
        consultarTarefas();
        consultarTotais();
    }
    ...
    return (
        ...
        <main class="mt-[30px] mx-[30px]">
            ...
            <div class="flex justify-end">
                <h5 class="text-xl font-bold leading-none text-gray-900 dark:text-black">Total:{totalEmAndamento}</h5>
            </div>
            <br />
            <div class="w-full p-4 bg-white border border-gray-200 rounded-lg shadow-sm sm:p-8 dark:bg-gray-800 dark:border-gray-700">
                <div class="flex items-center justify-between mb-4">
            ...
            <div class="flex justify-end">
                <h5 class="text-xl font-bold leading-none text-gray-900 dark:text-black">Total:{totalConcluidas}</h5>
            </div>
